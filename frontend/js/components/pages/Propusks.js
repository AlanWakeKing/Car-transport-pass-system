...
export class PropusksPage {
  ...
  async loadData() {
    ...
    try {
      const propusks = await apiGet(ENDPOINTS.propusks, { ...filters, limit: 50 });
          ...
          <tr data-id="${p.id_propusk}">
            ...
            <td>
              <div class="table-actions">
                ${canControlStatus(this.context.state.user) && p.status === "draft" ? `<button class="md-btn tonal" data-action="activate">Активировать</button>` : ""}
                ...
                ${canControlStatus(this.context.state.user) && ["active","pending_delete"].includes(p.status) ? `<button class="md-btn ghost" data-action="revoke">Аннулировать</button>` : ""}
                ${canManagePropusks(this.context.state.user) ? `<button class="md-btn ghost" data-action="archive">Удалить</button>` : ""}
