...
export class PropusksPage {
      ...
      <div class="md-card">
        ...
        <div class="toolbar">
          ...
          <div>
            ...
            <h3 style="margin:0;">Реестр пропусков</h3>
  ...
  renderPagination(node) {
    ...
    const { page, total } = this.state.pagination;
  ...
  renderRows(tbody) {
    ...
    if (!this.state.propusks.length) {
      if (tbody) {
        tbody.innerHTML = `<tr><td colspan="6"><div class="empty">Нет записей</div></td></tr>`;
      }
      if (cards) {
        cards.innerHTML = `<div class="empty">Нет записей</div>`;
      }
      return;
    }
  ...
  bind(node) {
    ...
    node.querySelector("#propusk-rows")?.addEventListener("click", async (e) => {
      ...
      if (!id) return;
      try {
        await handleAction(id, actionBtn.dataset.action);
      } catch (err) {
        handleError(err);
      }
    });
    ...
    form.innerHTML = `
      <div class="form-grid">
        ...
        <div class="md-field" style="grid-column:1/-1;">
          ...
          <textarea class="md-textarea" name="info" placeholder="Комментарий"></textarea>
        </div>
...
`;
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        toast.show("Пропуск сохранён", "success");
    ...
    form.innerHTML = `
      ...
      <div class="propusk-edit-grid history-collapsed">
        ...
        <div class="form-grid">
          ...
          <div class="md-field" style="grid-column:1/-1;">
            <label>Комментарий</label>
            <textarea class="md-textarea" name="info" placeholder="Комментарий">${propusk.info || ""}</textarea>
          </div>
...
`;
    ...
    const instance = modal.show({ title: `Пропуск #${propusk.id_propusk}`, content: form });
  ...
  confirmArchive(id) {
    ...
    content.innerHTML = `
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="md-btn ghost" id="cancel-archive">РћС‚РјРµРЅР°</button>

        <button type="button" class="md-btn" id="confirm-archive">РЈРґР°Р»РёС‚СЊ</button>
