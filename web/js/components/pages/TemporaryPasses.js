...
export class TemporaryPassesPage {
  ...
  async render() {
    ...
    node.innerHTML = `
      ...
      <div class="md-card">
        ...
        <div class="mobile-cards mobile-only" id="temp-pass-cards"></div>
  ...
  renderRows(tbody) {
    ...
    const cards = this.host?.querySelector("#temp-pass-cards");
    ...
    this.renderCards(cards, this.state.passes);
    ...
    container.innerHTML = passes
      ...
      .map((p) => {
        ...
        const actions = `
          ...
          ${canCreateTempPass(this.context.state.user) && !p.entered_at ? `<button class="md-btn ghost" data-action="enter" data-id="${p.id}">Заехал</button>` : ""}

          ${canCreateTempPass(this.context.state.user) && p.entered_at && !p.exited_at ? `<button class="md-btn ghost" data-action="exit" data-id="${p.id}">Выехал</button>` : ""}

          ${canDeleteTempPass(this.context.state.user) ? `<button class="md-btn ghost" data-action="delete" data-id="${p.id}">Удалить</button>` : ""}

          ${canDeleteTempPass(this.context.state.user) && p.status === "active" ? `<button class="md-btn ghost" data-action="revoke" data-id="${p.id}">Отозвать</button>` : ""}

          ${canDownloadTempPass(this.context.state.user) ? `<button class="md-btn secondary" data-action="pdf" data-id="${p.id}">PDF</button>` : ""}
          ...
          <div class="mobile-card">
            ...
            <div class="mobile-card-header">
              ...
              <div>
                ...
                <div class="mobile-card-subtitle">#${p.id}</div>
            ...
            <div class="mobile-card-meta">
              ...
              <div class="mobile-card-row"><span> </span><span>${this.formatDateTime(p.entered_at)}</span></div>

              <div class="mobile-card-row"><span> </span><span>${this.formatDateTime(p.exited_at)}</span></div>

              <div class="mobile-card-row"><span></span><span>${p.phone || "-"}</span></div>
  ...
  bind(node) {
    ...
    node.querySelector("#temp-pass-cards")?.addEventListener("click", async (e) => {
    ...
    form.innerHTML = `
      ...
      <div class="form-grid">
        ...
        <div class="md-field" style="grid-column:1/-1;">
          ...
          <input class="md-input" name="org_name" list="temp-org-options-create" placeholder="Выберите организацию" required>
  ...
  confirmRevoke(id) {
    ...
    const instance = modal.show({ title: "Отозвать пропуск", content });
  ...
  confirmDelete(id) {
    ...
    content.innerHTML = `
      ...
      <p>Удалить пропуск #${id}? Это действие необратимо.</p>
    ...
    const instance = modal.show({ title: "Удаление пропуска", content });
