...
from models import User, PropuskTemplate, TemporaryPassTemplate, TemporaryPassReportTemplate
...
from settings.service import (
    ...
    get_active_temp_pass_template,
    list_recent_temp_pass_templates,
    save_temp_pass_template,
    get_active_temp_pass_report_template,
    list_recent_temp_pass_report_templates,
    save_temp_pass_report_template,
    get_api_enabled,
    set_api_enabled,
    get_docs_enabled,
    set_docs_enabled,
...
@router.get("/temporary-pass-template/active", response_model=PropuskTemplateResponse)
def get_active_temp_pass_template_view(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin),
):
...
@router.get("/temporary-pass-template/versions", response_model=list[PropuskTemplateResponse])
def list_temp_pass_template_versions(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin),
):
    templates = list_recent_temp_pass_templates(db, limit=2)
    result = []
    for template in templates:
        data = template.data_json if hasattr(template, "data_json") else json.loads(template.data or "{}")
        result.append(
...
@router.post("/temporary-pass-template", response_model=PropuskTemplateResponse, status_code=status.HTTP_201_CREATED)
def create_temp_pass_template(
    payload: PropuskTemplatePayload,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin),
):
    template = save_temp_pass_template(db, payload.data, current_user.id)
    data = template.data_json if hasattr(template, "data_json") else json.loads(template.data or "{}")
    return {
        "id": template.id,
        "version": template.version,
...
@router.get("/temporary-pass-report-template/active", response_model=PropuskTemplateResponse)
def get_active_temp_pass_report_template_view(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin),
):
    template = get_active_temp_pass_report_template(db)
    if not template:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Template not found")
    data = template.data_json if hasattr(template, "data_json") else json.loads(template.data or "{}")
    return {
...
@router.get("/temporary-pass-report-template/versions", response_model=list[PropuskTemplateResponse])
def list_temp_pass_report_template_versions(
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin),
):
    templates = list_recent_temp_pass_report_templates(db, limit=2)
    result = []
    for template in templates:
        data = template.data_json if hasattr(template, "data_json") else json.loads(template.data or "{}")
        result.append(
...
@router.post("/temporary-pass-report-template", response_model=PropuskTemplateResponse, status_code=status.HTTP_201_CREATED)
def create_temp_pass_report_template(
    payload: PropuskTemplatePayload,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin),
):
    template = save_temp_pass_report_template(db, payload.data, current_user.id)
    data = template.data_json if hasattr(template, "data_json") else json.loads(template.data or "{}")
    return {
        "id": template.id,
        "version": template.version,
